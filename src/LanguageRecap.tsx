import MathField, { MathContent } from './MathField'
import './math.css'
...
type SectionType =
  "alphabet" | "vocabulary" | "rule" | "declension" | "conjugation";
...
type Section = {
  ...
  data: Record<string, unknown> & {
    web_resources?: SectionResource[];
    local_documents?: SectionResource[];
    table?: RecapTable;
    blocks?: RecapBlock[];
  };
  position: number;
...
const emptyForm = (position: number): SectionForm => ({
...
export default function LanguageRecap({ selectedLanguage }: Props) {
  ...
  const loadSections = async (nextLanguage: string) => {
    ...
    if (!editingId)
      setForm((current) => ({ ...current, position: nextPosition }));
  };
  ...
  useEffect(() => {
    void loadSections(language);
  }, [language]);

  const visible = sections.filter(
    (section) => section.section_type === activeType,
  );
  ...
  const resetForm = (nextLanguage: string = language) => {
    ...
    setForm({
      ...emptyForm(Math.max(1, sections.length + 1)),
      section_type: "vocabulary",
      title: "",
      content: "",
    });
  ...
  const startEditing = (section: Section) => {
    ...
    setForm({
      ...
      tableJson: section.data?.table
        ? JSON.stringify(section.data.table, null, 2)
        : "",
      blocksJson: Array.isArray(section.data?.blocks)
        ? JSON.stringify(section.data.blocks, null, 2)
        : "",
    });
  };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (form.tableJson.trim()) {
      try {
        ...
        setMessage(
          "Le tableau JSON est invalide. Vérifie le format avant d’enregistrer.",
        );
        return;
      }
    }
    ...
    if (form.blocksJson.trim()) {
      try {
        ...
        setMessage(
          "Les rectangles JSON sont invalides. Vérifie le format avant d’enregistrer.",
        );
        return;
      }
    }
    ...
    setMessage(
      editingId
        ? "Le récapitulatif a bien été mis à jour."
        : "Le nouveau récapitulatif a bien été ajouté.",
    );
    setEditingId(null);
    setShowAdminPanel(false);
  ...
  return (
    <section className="recap-page">
      ...
      <div className="recap-grid">
        {visible.length ? (
          visible.map((section) => (
            <article className="recap-card" key={section.id}>
              <div className="recap-card-heading">
                <span className="recap-icon">
                  <Table2 size={18} />
                </span>
                <div>
                  <p className="card-kicker">{labels[section.section_type]}</p>
                  <MathContent value={section.title} className="recap-card-title" />
                </div>
              </div>
              <MathContent value={section.content} className="recap-content" />
              {section.data.blocks?.map((block, index) => (
                <article className="recap-block" key={`${section.id}-block-${index}`}>
                  <MathContent value={block.title} />
                  <MathContent value={block.content} />
                </article>
              ))}
              {section.data?.table && (
                <div className="recap-table-wrapper">
                  <table className="recap-table">
                    <thead>
                      <tr>
                        {section.data.table.headers.map((header) => (
                          <th key={`${section.id}-header-${header}`}>
                            <MathContent value={header} />
                          </th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {section.data.table.rows.map((row, index) => (
                        <tr key={`${section.id}-row-${index}`}>
                          {row.map((cell, cellIndex) => (
                            <td
                              key={`${section.id}-cell-${index}-${cellIndex}`}
                            >
                              <MathContent value={cell} />
                            </td>
                          ))}
                        </tr>
                      ))}
                ...
                <button
                  ...
                  onClick={() =>
                    window.speechSynthesis.speak(
                      new SpeechSynthesisUtterance(section.content),
                    )
                  }
                >
                  <Volume2 size={15} /> Écouter le récapitulatif
                </button>
              )}
              {section.section_type === "alphabet" && (
                <div className="recap-note">
                  <BookOpen size={15} /> Utilisez le manuel complet dans
                  Supports pour la progression détaillée.
                </div>
              )}
              {typeof section.data.source_document === "string" && (
                <button
                  className="lesson-action"
                  onClick={() =>
                    window.open(
                      section.data.source_document as string,
                      "_blank",
                      "noopener,noreferrer",
                    )
                  }
                >
                  <FileText size={15} /> Ouvrir le manuel complet
                </button>
              )}
              {section.data.local_documents?.map((document) => (
                <a
                  className="resource-link"
                  href={document.url}
                  target="_blank"
                  rel="noreferrer"
                  key={`${section.id}-${document.url}`}
                >
                  <FileText size={14} /> {document.title}
                </a>
              ))}
              {section.data.web_resources?.length ? (
                <div className="recap-web-resources">
                  <p className="card-kicker">Ressources web</p>
                  {section.data.web_resources.map((resource) => (
                    <a
                      className="resource-link"
                      href={resource.url}
                      target="_blank"
                      rel="noreferrer"
                      key={`${section.id}-${resource.url}`}
                    >
                      <FileText size={14} /> {resource.title}
                    </a>
                  ))}
                </div>
              ) : null}
              {isAdmin && (
                <div className="recap-admin-inline-actions">
                  <button
                    type="button"
          ...
          <div className="empty-state">
            <FileText size={20} />
            <p>Ce récapitulatif sera enrichi depuis le manuel de {language}.</p>
          </div>
        )}
      </div>
      {isAdmin && (
        <div className="recap-admin-shell">
          <button
            type="button"
            className="recap-admin-toggle"
            onClick={() => setShowAdminPanel((current) => !current)}
          >
            {showAdminPanel ? "Masquer l’éditeur" : "Modifier le recap"}
          </button>
          {showAdminPanel && (
            <div className="recap-admin-panel">
              <div className="recap-admin-header">
                <div>
              ...
              >
                ...
                <label>
                  ...
                  <input
                    ...
                    placeholder="Ex. : Vocabulaire week 1"
                    required
                  />
                </label>
                <MathField
                  label="Contenu (LaTeX possible)"
                  value={form.content}
                  onChange={(content) => setForm((current) => ({ ...current, content }))}
                  rows={6}
                />
                <label>
                  Tableau JSON (optionnel)
                  <textarea
                    rows={6}
                    ...
                    onChange={(event) =>
                      setForm((current) => ({
                        ...current,
                        tableJson: event.target.value,
                      }))
                    }
                    placeholder={
                      '{"headers": ["Cas", "Masculin", "Féminin"], "rows": [["Nominatif", "der", "die"], ["Accusatif", "den", "die"]] }'
                    }
                  />
                </label>
